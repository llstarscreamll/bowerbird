...
func NewRequestUploadURLCommand(store platformStorage.FileStore) *RequestUploadURLCommand {
	return &RequestUploadURLCommand{store: store}
}
...
func (cmd *RequestUploadURLCommand) Execute(ctx context.Context, input RequestUploadURLInput) (*platformStorage.PresignUploadResult, error) {
	...
	return cmd.store.PresignUpload(ctx, platformStorage.PresignUploadInput{
		Path:        key,
		...
		Metadata: map[string]string{
			...
			"cache-control":       "max-age=31536000, public, immutable", // cache the file for 1 year
