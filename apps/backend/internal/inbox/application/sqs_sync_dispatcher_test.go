...
func (q *fakeJobQueue) Dispatch(ctx context.Context, job jobs.Job) error {
	q.jobs = append(q.jobs, job)
	return nil
}

func TestSQSSyncAccountJobDispatcher_EnqueuesInboxSyncAccount(t *testing.T) {
	queue := &fakeJobQueue{}
	dispatcher := inboxCommands.NewSQSSyncAccountJobDispatcher(queue)
