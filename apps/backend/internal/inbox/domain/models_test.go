...
func TestSyncCursorAdvanceHistoryRequiresValue(t *testing.T) {
	cursor, err := NewSyncCursor("conn-1", nil)
	if err != nil {
		t.Fatalf("new sync cursor: %v", err)
	}
	if err := cursor.AdvanceHistory(""); !errorsIs(err, ErrSyncCursorHistoryIDRequired) {
		t.Fatalf("expected history id required, got %v", err)
	}
	if err := cursor.AdvanceHistory("hist-1"); err != nil {
		t.Fatalf("advance history: %v", err)
	}
	if cursor.HistoryID() != "hist-1" {
		t.Fatalf("expected history id hist-1, got %q", cursor.HistoryID())
	}
}
