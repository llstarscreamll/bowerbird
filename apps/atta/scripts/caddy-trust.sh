...
readonly CADDY_CONTAINER_NAME="atta-caddy"
readonly CADDY_CA_PATH_IN_CONTAINER="/data/caddy/pki/authorities/local/root.crt"
readonly LINUX_SYSTEM_CA_TARGET_FEDORA="/etc/pki/ca-trust/source/anchors/atta-caddy-local-ca.crt"
readonly LINUX_SYSTEM_CA_TARGET_ARCH="/etc/ca-certificates/trust-source/anchors/atta-caddy-local-ca.crt"
readonly NICKNAME="Atta Caddy Local CA"
...
extract_caddy_root_ca() {
  ...
  if ! docker inspect "$CADDY_CONTAINER_NAME" >/dev/null 2>&1; then
    die "Container '$CADDY_CONTAINER_NAME' was not found. Start infra first (for example: mise //apps/atta:infra:up)."
  fi

  local running
  running="$(docker inspect -f '{{.State.Running}}' "$CADDY_CONTAINER_NAME" 2>/dev/null || true)"
  if [[ "$running" != "true" ]]; then
    die "Container '$CADDY_CONTAINER_NAME' is not running. Start infra first (for example: mise //apps/atta:infra:up)."
  fi
