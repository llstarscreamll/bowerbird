...
func (h *Router) Register(mux *http.ServeMux, cfg config.Config, authMiddleware func(http.Handler) http.Handler) {
	...
	mux.Handle("GET /api/v1/catalog/items/duplicate-clusters", authMiddleware(api.Wrap(h.controller.ListDuplicateClusters, cfg)))
	mux.Handle("POST /api/v1/catalog/items/merges", authMiddleware(api.Wrap(h.controller.MergeItems, cfg)))
	mux.Handle("POST /api/v1/catalog/items/not-duplicates", authMiddleware(api.Wrap(h.controller.MarkNotDuplicates, cfg)))
