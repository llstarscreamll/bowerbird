...
// Provisioner is responsible for the physical infrastructure provisioning of a new tenant.
type Provisioner interface {
	// CreateDatabase creates a new physical database.
	CreateDatabase(ctx context.Context, dbName string) error

	// MigrateDatabase applies the latest business schemas to the tenant's database.
