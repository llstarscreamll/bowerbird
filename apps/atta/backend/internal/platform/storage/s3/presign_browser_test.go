...
func TestPresignUploadViaMediaProxy(t *testing.T) {
	...
	presignEndpoint := "https://media.atta.dev"
	...
	store := platformS3.NewObjectStoreWithClients(s3Client, presignClient, "atta-local-bucket")
	...
	if !strings.Contains(result.URL, "media.atta.dev") {
