...
import (
	...
	"github.com/atta/internal/platform/awsconfig"
	"github.com/atta/internal/platform/id"
	platformStorage "github.com/atta/internal/platform/storage"
	platformS3 "github.com/atta/internal/platform/storage/s3"
)
...
func TestMinIOPutObjectWithSyncMetadata(t *testing.T) {
	...
	cfg, err := awsconfig.Load(ctx, "us-east-1", endpoint, "atta", "attasecret")
	if err != nil {
		t.Fatalf("load aws config: %v", err)
	}
	store := platformS3.NewObjectStore(awsconfig.NewS3Client(cfg, endpoint), "atta-local-bucket")
...
func TestMinIOCredentialMismatchErrors(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "wrong secret", accessKey: "atta", secretKey: "wrongsecret", wantSubstr: "Forbidden"},
		{name: "wrong access key", accessKey: "wrong", secretKey: "attasecret", wantSubstr: "Forbidden"},
	}
