...
REMOTE_DIR="${ONPREM_HOST_REMOTE_DIR:-/opt/atta}"
REL="${ONPREM_RELEASE:?}"
APP_IMAGE="atta-onprem-app:${REL}"
CADDY_IMAGE="atta-onprem-caddy:${REL}"
REMOTE_COMPOSE="${REMOTE_DIR}/apps/atta/deploy/onprem"
TARGET="${USER}@${ADDRESS}"
...
scp "${ssh_opts[@]}" -P "$PORT" \
  "$ROOT/apps/atta/deploy/onprem/docker-compose.yml" \
  "$ROOT/apps/atta/deploy/onprem/Caddyfile" \
