...
El banner fijo de instalación PWA en `app.component.ts` permanece visible sin opción de cerrar, interrumpe el flujo de trabajo y no sigue las prácticas actuales (engagement gate, dismiss con cooldown, pull model). Los usuarios recurrentes de Atta se benefician de instalar la app, pero la promoción debe ser contextual, respetuosa y medible.

## What Changes

- Extraer orquestador de **system notices** a paquete workspace `@canopy/system-notices` (`packages/ts/system-notices/`).
...
- Publicar `@canopy/system-notices` fuera del monorepo (npm registry público).
...
- `system-notices`: paquete `@canopy/system-notices` — orquestador reutilizable, port `SystemNotice`, host Angular.
...
| Área    | Módulos                                                                                                     |
| ------- | ----------------------------------------------------------------------------------------------------------- |
| Package | `packages/ts/system-notices/` → `@canopy/system-notices` (orchestrator, port, host, `provideSystemNotices`) |
...
| PWA     | `package.json` — dependency `@canopy/system-notices`                                                        |
