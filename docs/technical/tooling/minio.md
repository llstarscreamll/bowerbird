...
- Bootstrap: `apps/atta/backend/scripts/init-minio.sh` (runs via `minio-init` on `mise //apps/atta:infra:up`)
- Backend: `MINIO_ENDPOINT_URL=http://localhost:9000`
- Browser uploads/downloads: `S3_PRESIGN_ENDPOINT_URL=https://media.atta.dev` (Caddy → MinIO `:9000`)

Default credentials (`apps/atta/docker-compose.yml`):

| Variable                                        | Value               |
| ----------------------------------------------- | ------------------- |
| `AWS_ACCESS_KEY_ID` / `MINIO_ROOT_USER`         | `atta`              |
| `AWS_SECRET_ACCESS_KEY` / `MINIO_ROOT_PASSWORD` | `attasecret`        |
...
1. Confirm `apps/atta/.env` has `AWS_REQUEST_CHECKSUM_CALCULATION=when_required` and restart workers.
...
```bash
docker compose -f apps/atta/docker-compose.yml --profile init run --rm --no-TTY minio-init
```

## On-prem production

See [On-prem stack](../architecture/onprem-runtime.md) (`apps/atta/deploy/onprem/docker-compose.yml`). MinIO runs as an internal service; Caddy terminates HTTPS for API and media.
